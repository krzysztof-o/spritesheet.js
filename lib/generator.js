...
exports.generateData = function (files, options, callback) {
	var templateContent = fs.readFileSync(options.customFormat || (__dirname + '/../templates/' + options.format.template), 'utf-8');
	options.files = files;
	...
	fs.writeFile(options.path + '/' + options.name + '.' + options.extension, result, callback);
};
