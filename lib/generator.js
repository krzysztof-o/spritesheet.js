...
exports.generateData = function (files, options, callback) {
	...
	formats.forEach(function(format, i){
		...
		options.files.forEach(function (item, i) {
			...
			if (options.extrude) {
				var extrudeLeft = !item.trim.x ? options.extrude : 0;
				var extrudeTop = !item.trim.y ? options.extrude : 0;
				var extrudeRight = (item.trim.width - item.trim.x) === item.width  ? options.extrude : 0;
				var extrudeBottom = (item.trim.height - item.trim.y) === item.height ? options.extrude : 0;
				item.x += extrudeLeft;
				item.y += extrudeTop;
				item.width -= extrudeLeft + extrudeRight;
				item.height -= extrudeTop + extrudeBottom;
				item.trim.x -= extrudeLeft ? 0 : options.extrude;
				item.trim.y -= extrudeTop ? 0 : options.extrude;
				item.trim.width -= options.extrude * 2;
				item.trim.height -= options.extrude * 2;
			}
