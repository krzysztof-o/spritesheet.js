...
/**
 * Build the command used to extend the image border by a given "extrude". The extended border will have the color of the outer pixels.
 * @param {string} inputFile
 * @param {string} outputFile
 * @param {number} extrude
 */
var _extrudeCommand = function (inputFile, outputFile, extrude) {
    var doubleExtrude = extrude * 2;

    var resize = '"%[fx:w+' + doubleExtrude + ']x%[fx:h+' + doubleExtrude + ']"';
    var viewport = resize + '-' + extrude + '-' + extrude;
    var command = 'convert -define png:exclude-chunk=date "' + inputFile + '" -set option:distort:viewport ' + viewport + ' -virtual-pixel edge -filter point -distort SRT 0 +repage ' + outputFile;
...
exports.trimImages = function (files, options, callback) {
	if (!(options.trim || options.extrude !== 0)) return callback(null);
	...
	async.eachSeries(files, function (file, next) {
		...
		var trim = function (inputFile, outputFile, callback) {
			...
			var command = 'convert' + scale + ' ' + ' ' + fuzz + ' -define png:exclude-chunk=date "' + inputFile + '" -bordercolor transparent -border 1 -trim "' + outputFile + '"';
			exec(command, callback);
		};

		if (options.extrude) {
			exec(_extrudeCommand(file.originalPath, file.path, options.extrude), (err) => {
				if (err) return callback(err);
				trim(file.path, file.path, next);
			});
		} else {
			trim(file.originalPath, file.path, next);
		}
	}, callback);
};
	...
	exec('identify ' + filePaths.join(' '), function (err, stdout) {
		...
		sizes.forEach(function (item, i) {
			...
			if (options.trim) {
				...
				files[i].trimmed = forceTrimmed ||
				((files[i].trim.width + options.extrude * 2) !== files[i].width - options.padding * 2 ||
					(files[i].trim.height + options.extrude * 2) !== files[i].height - options.padding * 2);
			}
		});
		callback(null, files);
	});
...
exports.generateData = function (files, options, callback) {
	...
	formats.forEach(function(format, i){
		...
		options.files.forEach(function (item, i) {
			...
			if (options.extrude) {
				var trim = item.trim ? item.trim : {x: 0, y: 0, width: item.width, height: item.height};
				var extrudeLeft = !trim.x ? options.extrude : 0;
				var extrudeTop = !trim.y ? options.extrude : 0;
				var extrudeRight = (trim.width - trim.x) === item.width  ? options.extrude : 0;
				var extrudeBottom = (trim.height - trim.y) === item.height ? options.extrude : 0;
				item.x += extrudeLeft;
				item.y += extrudeTop;
				item.width -= extrudeLeft + extrudeRight;
				item.height -= extrudeTop + extrudeBottom;
				trim.x -= extrudeLeft ? 0 : options.extrude;
				trim.y -= extrudeTop ? 0 : options.extrude;
				trim.width -= options.extrude * 2;
				trim.height -= options.extrude * 2;
			}
