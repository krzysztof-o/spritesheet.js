...
var _extrudeCommand = function (inputFile, outputFile, extrude) {
    ...
    var command = 'convert -resize ' + resize + ' -define png:exclude-chunks=date "' + inputFile + '" -set option:distort:viewport ' + viewport + ' -virtual-pixel Edge -distort SRT 0 -filter point +repage ' + outputFile;
...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		var command = 'convert' + scale + ' ' + fuzz + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim "' + file.path + '"';
        ...
        if (options.extrude) {
            if (options.trim) {
                exec(command, function (err) {
                    ...
                    exec(_extrudeCommand(file.path, file.path, options.extrude), next);
                });
            } else {
                exec(_extrudeCommand(file.originalPath, file.path, options.extrude), next);
            }
        } else {
            exec(command, next);
        }
...
exports.generateData = function (files, options, callback) {
	...
	formats.forEach(function(format, i){
		...
		options.files.forEach(function (item, i) {
			item.width  -= (options.padding * 2 + options.extrude * 2);
			item.height -= (options.padding * 2 + options.extrude * 2);
